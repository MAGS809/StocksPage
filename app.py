...
# ---------------------------------------------------------------------------
# 4. NEWS SENTIMENT ANALYSIS  (read-only context, no trade signals)
# ---------------------------------------------------------------------------

def fetch_and_analyze_news(ticker: str) -> pd.DataFrame:
              """
                  Fetch the 5 most recent news headlines for the given ticker from
                      Yahoo Finance and run a basic TextBlob sentiment analysis on each.

                          Each headline is classified as Positive, Negative, or Neutral
                              based on its polarity score.
                                                              ...
                                                              Returns
                                                                  -------
                                                                      pd.DataFrame with columns: Headline, Source, Polarity, Sentiment
                                                                              Returns an empty DataFrame if no news is available.
                                                                                  """
    try:
...
def render_news_ui(ticker: str):
              """
                  Render the News Context section in the Streamlit UI.
                      ...
                      Displays recent headlines, their source, polarity score, and
                          sentiment label in a clean table. Purely informational.
    ...
    # Display table (hide Link column from styled view)
    ...
    # Show clickable links below the table
...
# ---------------------------------------------------------------------------
# 5. EDUCATIONAL EXPLAINER  (strictly informational - no recommendations)
# ---------------------------------------------------------------------------
